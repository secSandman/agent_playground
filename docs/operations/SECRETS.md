...
## ⚠️ Current Launcher Behavior (Important)

The active Python launchers (`opencode_run.py`, `claudecode_run.py`) fetch secrets on the host and inject them via environment variables at runtime.

- Dev mode: local Vault (`http://localhost:8200`, token `root`)
- Prod mode: expects `VAULT_TOKEN` to already be present in environment
- Current Python launchers do **not** perform interactive OIDC browser login automatically

If you use OIDC in production, obtain/export `VAULT_TOKEN` first, then run the launcher.
...
│ │ 2. Authenticate to secrets provider                  │ │
│ │ 3. Fetch ONLY specified secrets                      │ │
│ │ 4. Write to /tmp/opencode-secrets.env                │ │
│ │ 5. Exit                                              │ │
│ └──────────────────────────────────────────────────────┘ │
└─────────────────────────┬────────────────────────────────┘
                          │
...
│ - Has NO direct access to secrets provider               │
│ - Only sees specified credentials                        │
│ - Cannot fetch additional secrets                        │
└──────────────────────────────────────────────────────────┘
```
...
python opencode_run.py --workspace "C:\code\myproject" --prod-mode
```
