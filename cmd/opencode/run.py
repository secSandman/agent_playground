#!/usr/bin/env python3
"""
OpenCode Secure Sandbox Launcher - Cross-platform (Windows/macOS/Linux)

Run OpenCode CLI in a hardened container with Vault secrets management and Squid proxy.
...
def run_opencode(
    ...
    if isolation_mode != 'none':
        command = [
            ...
            '--tmpfs', '/home/opencodeuser/.local:rw,nosuid,size=64m',
            '--tmpfs', '/home/opencodeuser/.config:rw,nosuid,size=64m',
            '--tmpfs', '/home/opencodeuser/.cache:rw,nosuid,size=64m',
            '--cap-drop', 'ALL',
        ...
        command.append('opencode-sandbox:1.2.17')
    ...
    print(f"{Fore.CYAN}{'=' * 40}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}Starting OpenCode CLI{Style.RESET_ALL}")
    print(f"{Fore.CYAN}{'=' * 40}{Style.RESET_ALL}")
    print()
    ...
    try:
        ...
        print(f"{Fore.RED}[ERROR] Failed to run OpenCode: {e}{Style.RESET_ALL}")
        return 1
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='OpenCode Secure Sandbox Launcher',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python opencode_run.py --workspace ./test-workspace --dev-mode
    python opencode_run.py --workspace ./test-workspace --dev-mode --prompt "write hello world"
    python opencode_run.py --workspace ./code --prod-mode
        """
    )
    ...
    parser.add_argument(
        '--prompt',
        help='One-shot prompt mode: provide a question for OpenCode'
    )
    ...
    print_header(f"OpenCode Secure Sandbox Launcher")
    ...
    try:
        ...
        if not args.no_rebuild:
            if not docker_manager.build('opencode'):
